...
void average_data(struct data* data){
    int rows = data->no_of_rows;
    int columns = data->no_of_columns;
    int r, c;
    double average_x = 0;
    double average_y = 0;
    ...
    for(r = 0; r < rows; r++){
        for(c = 0; c < columns; c++){
            ...
            average_x += data->data_x[( r * columns) + c];
            average_y += data->data_y[( r * columns) + c];
        }

        data->data_x_mean[r] = (average_x / columns);
        data->data_y_mean[r] = (average_y / columns);
